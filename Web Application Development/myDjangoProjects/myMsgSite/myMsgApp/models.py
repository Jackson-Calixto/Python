...
class Person(models.Model):
    ...
    # Model method for returning a person's full name
    def _get_full_name(self):
        "Returns a person's full name."
        return '%s %s' % (self.first_name, self.last_name)
    # Add person's full name as a new managed attribute
    full_name = property(_get_full_name)
    # Add a __str__ method to return the full name of the person in UI
    def __str__(self):
...
class Mail(models.Model):
    ...
    person = models.ForeignKey(Person, on_delete=models.CASCADE) #many-to-one relationship (one person can send multiple mail messages)
    # Add a "__str__" method to return more detailed info about an individual message
    def __str__(self):
